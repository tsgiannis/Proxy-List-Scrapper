"""
 ...
 23-01-2025 Some minor modification in order to check if Categories matter
"""
...
class Proxy(object):
    ...
    """
    def __init__(self, category, ip, port):
        """
...
class Scrapper:
    ...
    def getProxies(self):
        ...
        if self.Categories[self.category] == 'ALL':
            ...
            self.filter_proxies_remove_duplicates()
             ...
             self.proxies = self._get()
        self.proxies = [Proxy(proxy['category'],proxy['proxy'].split(':')[0], proxy['proxy'].split(':')[1]) for proxy in self.proxies]
    ...
    def filter_proxies_remove_duplicates(self):
        ...
        for proxy in self.proxies:
         # Ensure the item is a dictionary and has the key 'proxy'
            if isinstance(proxy, dict) and 'proxy' in proxy:
                proxy_address = proxy['proxy']
                if proxy_address not in unique_proxies_set:
                    unique_proxies_set.add(proxy_address)
